...
class CalculadoraInterativa {
    ...
    criarGrafico(dados) {
        ...
        const defaultColors = [
            'rgba(102, 126, 234, 0.6)', // Azul claro
            'rgba(40, 167, 69, 0.6)',  // Verde
            'rgba(255, 193, 7, 0.6)',  // Amarelo
            'rgba(220, 53, 69, 0.6)'   // Vermelho
        ];
        ...
        this.graficoAtual = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: dados.label,
                    data: dados.valores,
                    backgroundColor: dados.cores || defaultColors,
                    borderColor: dados.coresBorda || defaultBorderColors,
                    borderWidth: 2
                }]
            },
            options: {
                ...
                plugins: {
                    legend: {
                        display: dados.tipo === 'pie' || dados.tipo === 'doughnut' // Exibe legenda apenas para gráficos circulares
                    }
                },
                scales: {
                    y: {
                        beginAtZero: true,
                        display: dados.tipo !== 'pie' && dados.tipo !== 'doughnut' // Oculta eixo Y para gráficos circulares
                    },
                    x: {
                        display: dados.tipo !== 'pie' && dados.tipo !== 'doughnut' // Oculta eixo X para gráficos circulares
                    }
                }
            }
        });
    ...
    adicionarAoHistorico(dados) {
        // Ignorar se o valor for apenas o placeholder (e.g., total=0)
        if (dados.valor === '0,00 R$') return; 
    ...
    verDetalheHistorico(index) {
        ...
        let detalhesTexto = item.detalhes.map(d => `${d.label}: ${d.valor}`).join('\n');
    ...
    obterTituloCalculo(tipo) {
        const titulos = {
            ...
            'gasto-lanche': 'Estimativa de Gasto no Pedido' // NOVO TÍTULO
        };
    ...
    validarInputs(ids) {
        for (let id of ids) {
            ...
            elemento.style.borderColor = ''; // Limpa a borda vermelha após validação bem-sucedida
        }
        return true;
    }
}
...
/**
 * CALCULAR RAÇÃO (Exemplo de cálculo antigo, mantido por compatibilidade)
...
/**
 * CALCULAR GASTO LANCHE (Cálculo personalizado para Breno Food)
 * Estima o custo total de um pedido.
 */
function calcularGastoLanche() {
    // 1. Validar inputs (apenas os obrigatórios)
    if (!calculadora.validarInputs(['preco-lanche', 'quantidade-lanche'])) {
        return;
    }

    // 2. Obter valores (os campos de Entrega e Gorjeta são opcionais e assumem 0 se vazios)
    const precoLanche = parseFloat(document.getElementById('preco-lanche').value) || 0;
    const quantidade = parseInt(document.getElementById('quantidade-lanche').value) || 0;
    const taxaEntrega = parseFloat(document.getElementById('taxa-entrega').value) || 0;
    const gorjetaPerc = parseFloat(document.getElementById('gorjeta-perc').value) || 0;

    // 3. Cálculo
    const subtotal = precoLanche * quantidade;
    const valorGorjeta = subtotal * (gorjetaPerc / 100);
    const totalGasto = subtotal + taxaEntrega + valorGorjeta;

    // 4. Formatação para Reais (BRL)
    ...
    // Classificação
    let classificacaoTexto = 'Pedido Normal';
    let classificacaoClasse = 'classificacao-normal';
    if (totalGasto > 100) {
        classificacaoTexto = 'Pedido Grande!';
        classificacaoClasse = 'classificacao-perigo';
    } else if (totalGasto > 50) {
        classificacaoTexto = 'Pedido Médio';
        classificacaoClasse = 'classificacao-alerta';
    }

    // 5. Preparar dados para exibição
    const dados = {
        tipo: 'gasto-lanche',
        valor: formatadorBRL.format(totalGasto), // Total formatado
        classificacao: {
            texto: classificacaoTexto,
            classe: classificacaoClasse
        },
        detalhes: [
            { label: 'Subtotal Lanches', valor: formatadorBRL.format(subtotal) },
            { label: 'Taxa de Entrega', valor: formatadorBRL.format(taxaEntrega) },
            { label: `Gorjeta (${gorjetaPerc}%)`, valor: formatadorBRL.format(valorGorjeta) },
            { label: 'Preço Unitário', valor: formatadorBRL.format(precoLanche) },
            { label: 'Quantidade', valor: `${quantidade} unidade(s)` }
        ],
        grafico: {
            tipo: 'pie', // Gráfico de pizza para visualização de proporção
            label: 'Composição do Gasto',
            labels: ['Lanches', 'Entrega', 'Gorjeta'],
            valores: [subtotal, taxaEntrega, valorGorjeta],
            cores: [
                'rgba(40, 167, 69, 0.6)', // Verde (Lanches)
                'rgba(102, 126, 234, 0.6)', // Azul (Entrega)
                'rgba(255, 193, 7, 0.6)' // Amarelo (Gorjeta)
            ]
        }
    };
